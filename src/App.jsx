...
export default function App() {
  const [mobileTab, setMobileTab] = useState('video'); // 'video' | 'suite'
  const [suiteTab, setSuiteTab] = useState('whiteboard'); // 'whiteboard' | 'files'
  ...
  return (
    <div className="app-container">
      ...
      <header className="navbar-container">
        <div className="brand-title">SyncSpace Studio</div>
        <div className="room-badge">Room: room-1</div>
      </header>

      {/* Mobile-Only Top Switcher Bar */}
      <div className="mobile-view-tabs">
        ...
        <button
          className={`mobile-tab-btn ${mobileTab === 'suite' && suiteTab === 'whiteboard' ? 'active' : ''}`}
          onClick={() => {
            setMobileTab('suite');
            setSuiteTab('whiteboard');
          }}
        >
          🎨 Board
        </button>
        <button
          className={`mobile-tab-btn ${mobileTab === 'suite' && suiteTab === 'files' ? 'active' : ''}`}
          onClick={() => {
            setMobileTab('suite');
            setSuiteTab('files');
          }}
      ...
      {/* Main Workspace Area */}
      <div className={`workspace-container ${mobileTab === 'video' ? 'show-video' : 'show-suite'}`}>
        {/* Stage / Video View */}
        ...
        {/* Side Suite / Whiteboard & Files */}
        <aside className="side-suite">
          <div className="side-suite-header">
            <button
              className={suiteTab === 'whiteboard' ? 'active' : ''}
              onClick={() => setSuiteTab('whiteboard')}
            >
              Whiteboard
            </button>
            <button
              className={suiteTab === 'files' ? 'active' : ''}
              onClick={() => setSuiteTab('files')}
            >
              Files
            </button>
          </div>

          <div className="side-suite-content">
            {suiteTab === 'whiteboard' ? (
              <div className="whiteboard-container">
                <p style={{ padding: '16px', color: '#94a3b8' }}>Whiteboard Canvas Area</p>
              </div>
            ) : (
              <div className="files-container">
                <p style={{ padding: '16px', color: '#94a3b8' }}>Shared Files List</p>
              </div>
            )}
          </div>
        </aside>
